...
注意到$(\bm{\Lambda}+\alpha\bm{I})^{-1}\bm{\Lambda}=\text{diag}([\frac{\lambda_1}{\lambda_1+\alpha},\cdots,\frac{\lambda_n}{\lambda_n+\alpha}])$，其中$\lambda_i$是$\bm{H}$的第$i$个特征值。因此变换的效果是以$\frac{\lambda_i}{\lambda_i+\alpha}$为比例系数缩放$\bm{w}$在$\bm{H}$的第$i$个特征向量上的分量。
