...
在提出DNC之后不久，发现效率太低的这帮人紧跟着对NTM，DNC中的一些共性操作做了稀疏化。单个操作速度快了$$1000\times$$，使用的内存少了$$3000\times$$。新模型发表在2016年的NIPS[^NIPS2016_6298]上。加速的最大意义是使得模型可以处理更大的，贴近现实的内存大小，从而使得记忆增强神经网络接近实用。Reviewer 5的原话如下：

>  The drastic efficiency and performance improvements reported in the present paper are likely to mean that the memory augmented neural networks are now ready to be used in practice. - Reviewer 5
...
0写操作相较读操作要更复杂，除了同样使用了相似度寻址，还引入了内存的释放等机制。这个机制涉及维护每个内存位置的usage信息，基于该信息找到被使用最少或者空闲的内存位置进行写入。
...
其中$$\lambda$$是折扣因子。这个式子的含义是，如果一个位置很久既没有读操作也没有写操作就应该是空闲的。相比原始DNC中读的强度越大，usage反而越低的设定，我倒是觉得更合理。
...
考虑的是最近的一次显著的读或者写究竟过去了多久。后面会介绍这里使用了$$O(1)$$时间复杂度的数据结构。
...
为了能够重建矩阵，我们当然还要存储非零元素的位置。因此用一个与`val`等长的数组`col_index`记录这些非零元的列数。并且用一个指针数组` row_ptr `记录每行第一个非零元的在`val`中的位置。` row_ptr `最后一个元素为`val`的长度加1。
